...
#   1. start a GPU workload against the ollama-router (unless SKIP_WORKLOAD=1)
...
ROUTER_URL="${ROUTER_URL:-http://localhost:11433}"
MODEL="${MODEL:-qwen3.6:35b-a3b-agentic}"
...
if [[ "$SKIP_WORKLOAD" == 1 ]]; then
    ...
    if ! curl -sf "$ROUTER_URL/api/tags" >/dev/null 2>&1; then
        echo "!! ollama-router not reachable at $ROUTER_URL"
