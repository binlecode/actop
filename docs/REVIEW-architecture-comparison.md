...
2. **Richer fan telemetry** — ✅ **closed in v1.2.3.** `smc.py`'s discovery now also reads `F{n}Mx` (max RPM) alongside `F{n}Ac`, and `read_fan_info()` returns `FanReading(current, max)` per fan (`docs/SPEC-system.md` §4.3), matching macmon's read-only current+max. `F{n}Mn` (min) is deliberately skipped — peers use it only to clamp fan-set *writes* actop doesn't perform.
