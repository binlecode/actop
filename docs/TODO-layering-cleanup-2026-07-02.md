...
Status: **LC-1 / LC-2 / LC-3 shipped; LC-4 open** · Created: 2026-07-02 ·
Status verified against `main` 2026-07-29 (v1.6.0)
...
> **Why this doc landed late.** The plan was written 2026-07-02 and drove LC-1
> through LC-3, but it lived only on the `lc-2-process-sample` branch and was
> never merged — the code shipped while the plan stayed unpublished. It is landed
> here as the design record for work already in `main`, plus the three items that
> genuinely remain (**§10**). Read §§1–9 as the originating plan (kept verbatim,
> so its line references point at July code, not current line numbers) and §10 as
> the only open scope.
...
| **LC-3** ✅ shipped | F3 (extend `analytics.py`: `AlertEngine`, throttling, session energy; widget analytics deleted). Note `analytics.py` now **exists** (created in LC-2 with `attribute_power`); LC-3 adds the alert/throttle/energy analytics to it | patch |
| **LC-4** *(optional)* — **open, see §10** | F6 (`RollingStats`) | patch |
...
## 10. Remaining work (verified against `main` at v1.6.0, 2026-07-29)

LC-1/LC-2/LC-3 are in `main`: `MetricsUpdated` carries exactly the snapshot
(§8-3), and every internal listed in §8-5 is gone from `tui/` — `analytics.py`
holds `attribute_power`, `domain_throttling`, `bandwidth_percent`,
`package_power_percent`, `AlertFrame`, and `AlertEngine`. Three items are open.

### 10.1 LC-4 / F6 / V8 — native-unit rolling stats still live in the widget

**Open, accepted debt.** `HardwareDashboard` still owns the watt and GB/s
history deques (`_cpu_w_hist`, `_gpu_w_hist`, `_pkg_w_hist`, `_bw_gbps_hist`)
and the `_avg_max` reducer behind the `avg N · max N` suffixes. F6's
`analytics.RollingStats` was never built.

Still the lowest-payoff item in the set — it relocates working code rather than
fixing a user-visible defect or unblocking a consumer. Priority is genuinely low;
the reason to do it is §10.2, which needs the same numbers outside the TUI.

### 10.2 Export parity gap — processes, alerts, and session energy are TUI-only

**Open, and the highest-value item here.** §8-4 deferred this as "documented
follow-up"; the documentation is this section. `export.py` emits nothing for the
three data points the TUI computes or opts into:

| Data point | Where it exists | Why export misses it |
|---|---|---|
| Processes | `SystemSnapshot.processes` | On the snapshot, so `snapshot_to_dict` would carry it — but `run_json_stream` / `serve_prometheus` construct `Monitor(interval_s, subsamples)` without `include_processes=True`, so the list is always empty |
| Alerts / throttle flags | `analytics.AlertFrame`, via `AlertEngine.feed()` | `AlertEngine` is instantiated by the dashboard widget; no export path drives it |
| Session energy | `AlertFrame.session_energy_j` | Same — the integral is only advanced by the TUI's per-frame `feed()` |

This is the concrete cost the plan's §1 predicted ("`export.py` can only emit what
`SystemSnapshot` carries"). It matters most for actop's core audience: a user
profiling a local inference run via `--json` or `--serve` gets watts and
utilization but cannot see which process drew them, whether the chip throttled,
or total energy for the run — exactly the questions a profiling session asks.

Two viable shapes, not yet decided:

1. **Opt-in flags on the export entry points** — `--json-processes` /
   `--serve-processes` pass `include_processes=True`, and the run loops own an
   `AlertEngine` whose `AlertFrame` is merged into the emitted record. Smallest
   change; leaves `SystemSnapshot` alone.
2. **Alerts become snapshot fields** — `AlertEngine` moves behind `Monitor`, so
   `SystemSnapshot` carries the throttle/alert booleans and session energy, and
   every consumer (TUI, NDJSON, Prometheus) reads one contract. Truer to §1's
   thesis, but makes `Monitor` stateful across calls and needs a decision about
   whether an API user wants alert sustain-counting they did not configure.

Prefer (1) unless a consumer materially needs alerts on the typed model —
`AlertEngine` is stateful and threshold-configured, which sits awkwardly inside a
per-frame snapshot. Note the per-PID Prometheus cardinality constraint recorded in
the v1.5.0 notes: process data stays NDJSON-only either way, never a labelled
gauge.

### 10.3 §8-1's literal grep does not pass — by design, not a defect
...
`DashboardConfig` (`create_dashboard_config(args, get_soc_info())`).

The acceptance criterion was written to kill **per-frame** L1 acquisition in the
view, and that is gone: nothing in the render loop touches L1. What remains is a
single startup call to obtain the platform info the config merge requires, which
has nowhere better to live — `create_dashboard_config` takes `soc_info` as a
parameter precisely so the caller supplies it. **Treat §8-1 as satisfied in
substance and reword it rather than "fixing" the import**; routing it through
another module to beat a grep would add indirection for no layering gain.
