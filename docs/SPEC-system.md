...
- `q`: Quit.
- `Space`: Pause / resume the sampling thread.
- `s`: Cycle process sorting column (`CPU%` \u2192 `GPU%` \u2192 `PWR` \u2192 `RSS` \u2192 `PID`).
...
- `p`: Show/hide the top processes table.
- `t`: Cycle the Textual app theme through a curated 8-theme cycle (`textual-dark` \u2192 `nord` \u2192 `dracula` \u2192 `tokyo-night` \u2192 `monokai` \u2192 `gruvbox` \u2192 `catppuccin-mocha` \u2192 `textual-light` \u2192 back). A toast notification flags each change. Orthogonal to `--palette` (chart gradient).
...
- **Color palettes** (`_PALETTES` / `--palette`, v1.4.1): the gradient stops are selectable for chart data visualization. Three palettes ship: `thermal` (default — blue→red gradient), `viridis` (colorblind-safe, 5-stop perceptual ramp), and `mono` (grayscale intensity). The palette is set once at construction from `DashboardConfig.palette` and passed eagerly into every `BrailleChart`.

- **App themes** (`--theme` / `t` key, v1.7.0): the Textual-level UI chrome (header, footer, borders, text colors, surface/background) is controlled by Textual's built-in `Theme` system — orthogonal to `--palette`, which controls chart gradient colors only. Eight themes are curated for `--theme` and the `t` runtime cycle: `textual-dark` (default), `nord`, `dracula`, `tokyo-night`, `monokai`, `gruvbox`, `catppuccin-mocha`, `textual-light`. Setting `App.theme` triggers `_watch_theme()` → CSS invalidation + refresh; the existing CSS already uses `$accent`, `$surface`, `$text-muted`, etc., which resolve from the active theme's `ColorSystem`. The `t` key cycles through the curated set, wrapping around, with a toast notification on each change. The two are independent: any theme works with any palette.
